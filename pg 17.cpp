...
int request_resources(int process_num, int request[]) {
    ...
    for (int i = 0; i < MAX_RESOURCES; i++) {
        ...
        allocation[process_num][i] += request[i]; // Update allocation matrix
        maximum[process_num][i] -= request[i];    // Update maximum matrix
    }
...
int is_safe() {
    ...
    int finish[MAX_PROCESSES] = {0}; // Finish array to mark processes that are finished
            ...
            if (finish[i] == 0) {
                ...
                for (j = 0; j < MAX_RESOURCES; j++) {
                    if (maximum[i][j] - allocation[i][j] > work[j]) {
                        break; // Cannot allocate resources to process i
                    }
                }
                if (j == MAX_RESOURCES) {
